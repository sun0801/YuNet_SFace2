...
def get_args():
    ...
    parser.add_argument("--skip_frames", help='number of frames to skip', type=int, default=5)
...
def match(recognizer, feature1, dictionary, directory):
    ...
    if best_match_user_id:
        return True, (best_match_user_id, max_score)
...
def main():
    ...
    cap = cv.VideoCapture(os.path.join(directory, "debug.mp4"))
    ...
    while True:
        ...
        image = cv.resize(image, dsize=(cap_width,cap_height))
        ...
        for face in faces:
            ...
            matched, user = match(face_recognizer, feature, dictionary, directory)

            box = list(map(int, face[:4]))
            # color = (255, 0, 0) if matched else (0, 0, 255)
